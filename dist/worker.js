// index.ts
var RMI_TOOLS = {
  urlcheck: { name: "urlcheck", description: "URL and domain safety check for crypto projects. Detects phishing sites, fake docs, and malicious redirects.", price: "$0.01", priceAtomic: "10000", category: "security", trialFree: 3, method: "POST" },
  pulse: { name: "pulse", description: "Market pulse dashboard - real-time token momentum, volume spikes, whale alerts, and trending tokens across all major chains.", price: "$0.01", priceAtomic: "10000", category: "market", trialFree: 3, method: "POST" },
  rugshield: { name: "rugshield", description: "Real-time rug protection score for any token. Multi-factor risk assessment including contract, liquidity, holder, and social signals.", price: "$0.02", priceAtomic: "20000", category: "security", trialFree: 3, method: "POST" },
  audit: { name: "audit", description: "Deep smart contract audit and vulnerability analysis for tokens on EVM chains. Detects honeypots, rugpull risks, hidden mint functions, and ownership traps.", price: "$0.05", priceAtomic: "50000", category: "security", trialFree: 1, method: "POST" },
  ...
  launch: { name: "launch", description: "Token launch analysis - bonding curve status, liquidity locks, initial holder distribution, and fair launch verification.", price: "$0.03", priceAtomic: "30000", category: "launchpad", trialFree: 2, method: "GET" },
  wallet: { name: "wallet", description: "Comprehensive wallet analysis - balance, token holdings, transaction history, risk score, and behavioral patterns.", price: "$0.05", priceAtomic: "50000", category: "analysis", trialFree: 1, method: "POST" },
  smartmoney: { name: "smartmoney", description: "Track smart money wallets and whale movements across chains. Find what profitable traders are buying before the crowd.", price: "$0.05", priceAtomic: "50000", category: "intelligence", trialFree: 1, method: "GET" },
  ...
  forensics: { name: "forensics", description: "Deep token forensics report combining DexScreener, GeckoTerminal, CoinGecko, DefiLlama, CryptoPanic. Returns risk score, liquidity analysis, pair age, verification status, and buy/sell recommendation.", price: "$0.10", priceAtomic: "100000", category: "analysis", trialFree: 1, method: "POST" },
  whale: { name: "whale", description: "Advanced whale wallet decoder - Solana/EVM balance, TX pattern analysis, DexScreener pair association, Blockchair multi-chain stats. Identifies whale persona, activity level, trust score.", price: "$0.15", priceAtomic: "150000", category: "intelligence", trialFree: 1, method: "POST" },
  launch_intel: { name: "launch_intel", description: "Token launch intelligence - PumpFun new tokens, DexScreener trending pairs, bonding curve tracking, market summary, risk alerts for coordinated launches.", price: "$0.05", priceAtomic: "50000", category: "launchpad", trialFree: 2, method: "POST" },
  ...
  honeypot_check: { name: "honeypot_check", description: "Honeypot detection - simulates buy/sell transactions to verify you can actually sell a token. Tests transfer taxes, sell locks, blacklists, and trap mechanisms.", price: "$0.05", priceAtomic: "50000", category: "security", trialFree: 2, method: "POST" },
  ...
  risk_monitor: { name: "risk_monitor", description: "Real-time risk monitoring - set alerts for wallets, tokens, or chains. Gets notified of rug pulls, liquidity removals, whale dumps, or contract changes.", price: "$0.05", priceAtomic: "50000", category: "security", trialFree: 1, method: "POST" },
  // Gap fillers - nobody else is doing these
  defi_yield_scanner: { name: "defi_yield_scanner", description: "DeFi yield scanner - finds best APYs across chains, detects unsustainable yields, checks protocol TVL trends, impermanent loss risk, and smart contract age.", price: "$0.08", priceAtomic: "80000", category: "market", trialFree: 1, method: "POST" },
  nft_wash_detector: { name: "nft_wash_detector", description: "NFT wash trading detector - identifies fake volume in NFT collections, tracks floor price manipulation, finds collections with real organic demand vs artificial hype.", price: "$0.10", priceAtomic: "100000", category: "analysis", trialFree: 1, method: "POST" },
...
var MCP_TOOLS = {};
var TOOLS_CACHED_AT = 0;
async function fetchMCPTools(env) {
  const now = Date.now();
  if (TOOLS_CACHED_AT > 0 && now - TOOLS_CACHED_AT < 3e5) return MCP_TOOLS;
  try {
    const headers = {};
    ...
    if (resp.ok) {
      ...
      console.error("MCP tools fetch returned", resp.status);
    }
  } catch (e) {
...
async function executeToolDirect(toolName, body, urlParams, env) {
  ...
  if (toolName === "audit" || toolName === "rugshield") {
    ...
    return await executeSmartMoney(chain, body?.threshold || 1e4, body?.limit || 20);
    ...
    return await executeHoneypotCheck(body?.token_address || body?.address || "", body?.chain || "base");
    ...
    return await executeCopyTradeFinder(body?.chain || "base", body?.min_win_rate || 0.6, body?.limit || 10);
  } else if (toolName === "token_comparison") {
    return await executeTokenComparison(body?.tokens || [], body?.chain || "base");
  } else if (toolName === "risk_monitor") {
    return await executeRiskMonitor(body?.token_address || body?.address || "", body?.wallet || "", body?.chain || "base");
  } else if (toolName === "defi_yield_scanner") {
    return await executeDefiYieldScanner(body?.chain || "all", body?.min_apy || 0, body?.min_tvl || 1e5);
  } else if (toolName === "nft_wash_detector") {
    return await executeNftWashDetector(body?.collection || "", body?.chain || "base");
    ...
    return await executeSniperAlert(body?.chain || "base", body?.min_liquidity || 1e3);
  } else if (toolName === "liquidity_flow") {
    return await executeLiquidityFlow(body?.chain || "base", body?.min_amount || 1e4);
  } else if (toolName === "rug_pull_predictor") {
    return await executeRugPullPredictor(body?.token_address || body?.address || "", body?.chain || "base");
    ...
    const epMap = { profile_flip: "profile-flips", fresh_pair: "fresh-pairs", clone_detect: "clones" };
  ...
  if (toolName === "liquidity_depth") return await executeLiquidityDepth(address || body?.token || "", chain, body?.amount || 1e3);
...
async function executeDeployerHistory(address, chain) {
  const result = { tool: "deployer_history", address, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], contracts: [], risk_flags: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=" + address);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs) {
        const deployerPairs = data.pairs.filter(
          (p) => (p.chainId === chain || chain === "all") && p.pairCreatedAt
        );
        for (const p of deployerPairs.slice(0, 25)) {
          ...
          if (p.liquidity?.usd < 1e3) result.risk_flags.push("Low liquidity token: " + pair.token);
          if (p.volume?.h24 === 0 && Date.now() - p.pairCreatedAt > 864e5) result.risk_flags.push("Dead token: " + pair.token);
        }
        result.sources_used.push("dexscreener");
      }
    }
  } catch (e) {
    console.debug("Deployer history DexScreener failed:", e);
  }
...
async function executeWhaleAccumulation(tokenAddress, chain) {
  const result = { tool: "whale_accumulation", token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], whale_activity: [], accumulation_score: 0 };
  try {
    ...
    if (resp.ok) {
      ...
      if (data.pairs && data.pairs.length > 0) {
        ...
        if (p.liquidity?.usd < 5e3 && buyVolume > 1e4) {
    ...
    console.debug("Whale accumulation failed:", e);
  }
...
async function executeLiquidityDepth(tokenAddress, chain, amount) {
  const result = { tool: "liquidity_depth", token: tokenAddress, chain, amount, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs && data.pairs.length > 0) {
        ...
        result.best_pool = result.pools.reduce((best, p) => p.liquidity_usd > (best?.liquidity_usd || 0) ? p : best, null);
        result.sources_used.push("dexscreener");
      }
    }
  } catch (e) {
    console.debug("Liquidity depth failed:", e);
  }
...
async function executeTokenAge(tokenAddress, chain) {
  const result = { tool: "token_age", token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs && data.pairs.length > 0) {
        ...
        const ages = pairs.map((p) => ({
          pair: p.pairAddress,
          dex: p.dexId,
          created: new Date(p.pairCreatedAt).toISOString(),
          age_hours: Math.round((Date.now() - p.pairCreatedAt) / 36e5),
          liquidity_usd: p.liquidity?.usd || 0,
          alive: (p.liquidity?.usd || 0) > 0
        }));
        result.pairs = ages;
        const oldest = ages.reduce((a, b) => a.age_hours > b.age_hours ? a : b, ages[0]);
        result.oldest_pair = oldest;
        const survivedCount = ages.filter((a) => a.alive && a.age_hours > 72).length;
        result.survival = {
    ...
    console.debug("Token age failed:", e);
  }
  return result;
}
async function executeWashTrading(tokenAddress, chain) {
  const result = { tool: "wash_trading", token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], findings: [], wash_score: 0 };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs && data.pairs.length > 0) {
        ...
        if (buys + sells > 1e3 && liquidity < 1e3) {
    ...
    console.debug("Wash trading failed:", e);
  }
  result.wash_probability = result.wash_score >= 80 ? "VERY_HIGH" : result.wash_score >= 50 ? "HIGH" : result.wash_score >= 25 ? "MODERATE" : "LOW";
  return result;
}
async function executeBundlerDetect(tokenAddress, chain) {
  const result = { tool: "bundler_detect", token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], signals: [], bundler_score: 0 };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs && data.pairs.length > 0) {
        const p = data.pairs[0];
        const ageHours = p.pairCreatedAt ? (Date.now() - p.pairCreatedAt) / 36e5 : 999;
        const buys = p.txns?.h24?.buys || 0;
        const sells = p.txns?.h24?.sells || 0;
        const volume = p.volume?.h24 || 0;
        const liquidity = p.liquidity?.usd || 0;
        ...
        if (ageHours < 6 && volume > 1e5 && liquidity < 5e3) {
    ...
    console.debug("Bundler detect failed:", e);
  }
...
async function executeArbitrageScan(token, chain) {
  const result = { tool: "arbitrage_scan", token, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], opportunities: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=" + token);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs && data.pairs.length >= 2) {
        const prices = data.pairs.filter((p) => p.priceUsd && parseFloat(p.priceUsd) > 0).map((p) => ({ dex: p.dexId, pair: p.pairAddress, price: parseFloat(p.priceUsd), liquidity: p.liquidity?.usd || 0, volume: p.volume?.h24 || 0 })).sort((a, b) => a.price - b.price);
        ...
        if (prices.length >= 2) {
          ...
          if (spread > 0.5) {
            result.opportunities.push({
              ...
              estimated_profit: (spread / 100 * 1e3).toFixed(2) + " USDC per $1,000 trade",
              viable: cheapest.liquidity > 1e3 && mostExpensive.liquidity > 1e3
            });
          }
        }
    ...
    console.debug("Arbitrage scan failed:", e);
  }
  result.opportunity_count = result.opportunities.length;
  return result;
}
async function executeScamDatabase(query, chain) {
  const result = { tool: "scam_database", query, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], matches: [], risk_flags: [] };
  ...
  if (/^0x[a-fA-F0-9]{40}$/.test(query) || /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(query)) {
    try {
      ...
      console.debug("Scam DB DexScreener failed:", e);
    }
  }
...
async function executeAudit(address, chain, toolName) {
  const result = { chain, address, sources_used: [] };
  try {
    ...
    console.debug("DexScreener failed:", e);
  }
  if (result.sources_used.length === 0) {
    try {
      const chainMap = { solana: "solana", base: "base", ethereum: "eth", bsc: "bsc" };
      ...
      console.debug("GeckoTerminal failed:", e);
    }
  }
  if (chain === "solana") {
    try {
      ...
      console.debug("Solana RPC failed:", e);
    }
  }
  const findings = [];
  let risk_score = 0;
  ...
  if (liq > 0 && liq < 1e3) {
    risk_score += 25;
    findings.push("Very low liquidity: $" + liq.toLocaleString());
  } else if (liq >= 1e3 && liq < 1e4) {
    risk_score += 15;
    findings.push("Low liquidity: $" + liq.toLocaleString());
  }
  const buyers = result.dexscreener?.buyers_24h || 0;
  const sellers = result.dexscreener?.sellers_24h || 0;
  if (buyers > 0 && sellers > buyers * 3) {
    risk_score += 20;
    findings.push("Sell pressure " + (sellers / buyers).toFixed(1) + "x");
  }
  if (result.sources_used.length === 0) {
    risk_score += 20;
    findings.push("No data from any source");
  }
  ...
  return {
    ...
    risk_level: level,
    findings,
    source_count: result.sources_used.length
  };
}
async function executeWallet(address, chain) {
  const result = { chain, address, sources_used: [], balance_sol: 0, tx_count_recent: 0 };
  if (chain === "solana") {
    try {
      const resp = await fetch("https://api.mainnet-beta.solana.com", {
        method: "POST",
      ...
      console.debug("Balance failed:", e);
    }
    try {
      const resp = await fetch("https://api.mainnet-beta.solana.com", {
        method: "POST",
      ...
      if (resp.ok) {
        ...
        result.success_rate = sigs.filter((s) => !s.err).length / Math.max(1, sigs.length);
        result.sources_used.push("solana_txs");
      }
    } catch (e) {
      console.debug("TXs failed:", e);
    }
  }
  const persona = result.balance_sol > 1e3 ? "whale" : result.balance_sol > 100 ? "experienced" : result.tx_count_recent > 0 ? "casual" : "inactive";
  return { tool: "Wallet Profiler", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), ...result, persona };
}
async function executeUrlCheck(url) {
  const warnings = [];
  ...
  try {
    ...
    for (const s of suspicious) {
      if (parsed.pathname.toLowerCase().includes(s)) warnings.push("Suspicious path: " + s);
    }
    return { tool: "URL Scam Detector", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), url, domain: parsed.hostname, risk_level: warnings.length > 0 ? "SUSPICIOUS" : "CLEAN", warnings, sources_used: ["url_analysis"] };
  } catch {
    return { tool: "URL Scam Detector", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), url, risk_level: "INVALID", warnings: ["Invalid URL"], sources_used: [] };
  }
}
async function executePulse(chain) {
  const trending = [];
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=");
    if (resp.ok) {
      const data = await resp.json();
      const pairs = (data.pairs || []).sort((a, b) => (b.volume?.h24 || 0) - (a.volume?.h24 || 0)).slice(0, 20);
      for (const p of pairs) {
        trending.push({ symbol: p.baseToken?.symbol, address: p.baseToken?.address, price_usd: p.priceUsd, volume_24h: p.volume?.h24 });
      }
    }
  } catch {
  }
  return { tool: "Token Pulse", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), chain, trending, sources_used: trending.length > 0 ? ["dexscreener"] : [] };
}
async function executeLaunch(chain) {
  const launches = [];
  if (chain === "solana") {
    try {
      ...
      if (resp.ok) {
        ...
        for (const c of (data || []).slice(0, 10)) {
          launches.push({ mint: c.mint, name: c.name, market_cap: c.usdMarketCap });
        }
      }
    } catch {
    }
  }
  return { tool: "Launch Radar", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), chain, new_launches: launches, sources_used: launches.length > 0 ? ["pumpfun"] : [] };
}
async function executeSmartMoney(chain, threshold, limit) {
  const tokens = [];
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=");
    if (resp.ok) {
      const data = await resp.json();
      const trending = (data.pairs || []).sort((a, b) => (b.volume?.h24 || 0) - (a.volume?.h24 || 0)).slice(0, limit);
      for (const p of trending) {
        tokens.push({ address: p.baseToken?.address, symbol: p.baseToken?.symbol, volume_24h: p.volume?.h24, liquidity: p.liquidity?.usd });
      }
    }
  } catch {
  }
  return { tool: "Smart Money Tracker", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), chain, threshold_usd: threshold, trending_tokens: tokens, sources_used: tokens.length > 0 ? ["dexscreener"] : [] };
}
async function executeCluster(address, chain) {
  return { tool: "Cluster Detection", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), address, chain, cluster_size: 0, related_wallets: [], sources_used: [], note: "Requires on-chain graph analysis" };
}
async function executeInsider(address, chain) {
  return { tool: "Insider Tracker", version: "2.0", timestamp: (/* @__PURE__ */ new Date()).toISOString(), address, chain, insider_signals: [], risk_level: "unknown", sources_used: [], note: "Requires historical transaction analysis" };
}
async function executeForensics(tokenAddress, chain) {
  const result = {
    token: tokenAddress,
    chain,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (data.pairs?.length > 0) {
        ...
        result.pair_age_hours = pair.pairCreatedAt ? (Date.now() - pair.pairCreatedAt) / 36e5 : 0;
        result.sources_used.push("dexscreener");
        if (result.liquidity_usd < 1e3) {
          riskScore += 30;
          result.risk_factors.push("Critical: Liquidity under $1K");
        } else if (result.liquidity_usd < 1e4) {
          riskScore += 15;
          result.risk_factors.push("Low liquidity under $10K");
        } else {
          result.positive_signals.push("Healthy liquidity: $" + result.liquidity_usd.toLocaleString());
        }
        if (result.pair_age_hours < 1) {
          riskScore += 25;
          result.risk_factors.push("Brand new pair (<1h)");
        } else if (result.pair_age_hours < 24) {
          riskScore += 10;
          result.risk_factors.push("Young pair (<24h)");
        } else {
          result.positive_signals.push("Established pair (" + Math.floor(result.pair_age_hours) + "h)");
        }
        if (result.sellers_24h > result.buyers_24h * 2) {
          riskScore += 20;
          result.risk_factors.push("Heavy sell pressure");
        }
      }
    }
  } catch (e) {
...
async function executeWhaleDecoder(walletAddress, chain) {
  const result = {
  ...
  if (chain === "solana") {
    try {
      ...
      if (resp.ok) {
        ...
        result.balance_sol = (data.result?.value || 0) / 1e9;
        result.sources_used.push("solana_rpc");
        if (result.balance_sol > 1e3) {
          result.whale_signals.push("Mega whale (>" + result.balance_sol.toFixed(1) + " SOL)");
          result.wallet_persona = "mega_whale";
        } else if (result.balance_sol > 100) {
          result.whale_signals.push("Whale (>" + result.balance_sol.toFixed(1) + " SOL)");
          result.wallet_persona = "whale";
        } else if (result.balance_sol > 10) {
          result.wallet_persona = "active_trader";
        } else {
          result.wallet_persona = "retail";
        }
      }
    } catch (e) {
      console.debug("Balance failed:", e);
    ...
    try {
      ...
      if (resp.ok) {
        ...
        result.success_rate = sigs.filter((s) => !s.err).length / Math.max(1, sigs.length);
        result.sources_used.push("solana_txs");
        const last24h = sigs.filter((s) => Date.now() - new Date(s.blockTime * 1e3).getTime() < 24 * 60 * 60 * 1e3);
  ...
  try {
    ...
    console.debug("DexScreener search failed:", e);
  }
...
async function executeLaunchIntel(chain, hours = 24) {
  const result = { chain, lookback_hours: hours, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], new_launches: [], market_summary: {}, risk_alerts: [] };
  if (chain === "solana") {
    try {
      ...
      if (resp.ok) {
        ...
        const recent = (data || []).filter((c) => c.created_timestamp * 1e3 > cutoff);
        for (const coin of recent.slice(0, 20)) {
          const launch = {
            ...
            created: new Date(coin.created_timestamp * 1e3).toISOString(),
            age_hours: (Date.now() - coin.created_timestamp * 1e3) / 36e5,
            king_of_hill: coin.kingOfTheHillTimestamp ? true : false,
            bonding_curve_progress: coin.bondingCurveProgress || 0,
            creator: coin.creator,
            risk_flags: []
          };
          if (launch.age_hours < 1) launch.risk_flags.push("brand_new");
          if (launch.market_cap < 1e4) launch.risk_flags.push("low_mc");
          if (launch.market_cap > 1e6) launch.risk_flags.push("high_mc");
      ...
      console.debug("PumpFun failed:", e);
    }
  }
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=");
    if (resp.ok) {
      const data = await resp.json();
      const pairs = (data.pairs || []).filter((p) => p.chainId === (chain === "solana" ? "solana" : "base"));
      result.top_pairs = pairs.sort((a, b) => (b.volume?.h24 || 0) - (a.volume?.h24 || 0)).slice(0, 10).map((p) => ({
        symbol: p.baseToken?.symbol,
        address: p.baseToken?.address,
        price_usd: p.priceUsd,
  ...
  result.market_summary = {
    ...
    avg_market_cap: result.new_launches.length > 0 ? result.new_launches.reduce((sum, l) => sum + (l.market_cap || 0), 0) / result.new_launches.length : 0,
    king_of_hill_count: result.new_launches.filter((l) => l.king_of_hill).length,
    high_risk_count: result.new_launches.filter((l) => l.risk_flags.includes("brand_new") || l.risk_flags.includes("low_mc")).length
  };
  if (result.market_summary.total_new_launches > 50) result.risk_alerts.push("Unusually high launch volume");
  if (result.market_summary.avg_market_cap > 5e5) result.risk_alerts.push("High average market cap");
  return result;
}
async function executeAnomalyDetector(chain = "all") {
  const result = { chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], anomalies: [], market_health: "normal", alerts: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=");
    if (resp.ok) {
      const data = await resp.json();
      const pairs = data.pairs || [];
      for (const pair of pairs.filter((p) => (p.volume?.h24 || 0) > 1e6).slice(0, 10)) {
      ...
      for (const pair of pairs.filter((p) => (p.liquidity?.usd || 0) < 1e3).slice(0, 10)) {
        result.anomalies.push({
          type: "low_liquidity",
          token: pair.baseToken?.symbol,
          address: pair.baseToken?.address,
          liquidity: pair.liquidity?.usd,
          severity: pair.liquidity?.usd < 100 ? "critical" : "warning",
  ...
  const extreme = result.anomalies.filter((a) => a.severity === "extreme").length;
...
async function executeSocialSignal(query) {
  const result = { query, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], signals: [], sentiment_score: 0, confidence: "low", recommendation: "insufficient_data" };
  try {
    ...
    console.debug("Twitter profile failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      if (match) {
        ...
        const tweets = entries.filter((e) => e.type === "tweet").slice(0, 20);
        result.recent_tweets = tweets.map((e) => {
          const t = e.content?.tweet || {};
          return { text: t.full_text || t.text || "", created: t.created_at, likes: t.favorite_count || 0, retweets: t.retweet_count || 0 };
        });
        const avgLikes = result.recent_tweets.reduce((sum, t) => sum + t.likes, 0) / Math.max(1, result.recent_tweets.length);
        result.engagement = { total_tweets: result.recent_tweets.length, avg_likes: avgLikes, quality: avgLikes > 1e3 ? "very_high" : avgLikes > 100 ? "high" : avgLikes > 10 ? "medium" : "low" };
        result.sources_used.push("syndication");
      }
    }
  } catch (e) {
    console.debug("Timeline failed:", e);
  }
  try {
    ...
    console.debug("CryptoPanic failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      const matched = coins.find(
        (c) => c.item?.name?.toLowerCase() === query.toLowerCase() || c.item?.symbol?.toLowerCase() === query.toLowerCase()
      );
      if (matched) {
    ...
    console.debug("CoinGecko trending failed:", e);
  }
  try {
    ...
    console.debug("LunarCrush failed (free tier):", e);
  }
...
async function executeMarketOverview(chain = "all") {
  const result = {
    chain,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    sources_used: [],
  ...
  try {
    ...
    console.debug("Blockchain.info failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      if (data.status === "1" && data.result) {
        result.eth = {
          ...
          timestamp: new Date(data.result.ethbtc_timestamp * 1e3).toISOString()
        };
        result.sources_used.push("etherscan");
      }
    }
  } catch (e) {
    console.debug("Etherscan failed:", e);
  }
  try {
    ...
    console.debug("BscScan failed:", e);
  }
  try {
    const resp = await fetch("https://api.llama.fi/chains");
    if (resp.ok) {
      const data = await resp.json();
      const chains = data.filter(
        (c) => ["Ethereum", "Solana", "Base", "BSC", "Arbitrum", "Optimism", "Polygon"].includes(c.name)
    ...
    console.debug("DefiLlama chains failed:", e);
  }
  try {
    const resp = await fetch("https://api.coingecko.com/api/v3/search/trending");
    if (resp.ok) {
      const data = await resp.json();
      result.trending = (data.coins || []).slice(0, 10).map((c) => ({
  ...
  try {
    ...
    if (resp.ok) {
      const data = await resp.json();
      result.top_volume = (data.Data || []).map((d) => ({
    ...
    console.debug("CryptoCompare failed:", e);
  }
  try {
    ...
    console.debug("Mempool failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      result.defi_stats = {
        ...
        total_tvl: data.reduce((sum, p) => sum + (p.tvl || 0), 0),
        top_protocols: data.sort((a, b) => (b.tvl || 0) - (a.tvl || 0)).slice(0, 5).map((p) => ({
          name: p.name,
          tvl: p.tvl,
          chain: p.chain,
          category: p.category
    ...
    console.debug("DefiLlama protocols failed:", e);
  }
  result.summary = {
...
async function executeTokenDeepDive(tokenSymbol, chain = "all") {
  const result = {
    query: tokenSymbol,
    chain,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  try {
    ...
    if (resp.ok) {
      const data = await resp.json();
      const coins = data.coins || [];
      const match = coins.find((c) => c.symbol?.toLowerCase() === tokenSymbol.toLowerCase()) || coins[0];
      if (match) {
    ...
    console.debug("CoinGecko failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      const data = await resp.json();
      const pairs = data.pairs || [];
      result.dex_pairs = pairs.slice(0, 10).map((p) => ({
        chain: p.chainId,
        dex: p.dex?.name,
  ...
  try {
    ...
    console.debug("CoinCap failed:", e);
  }
  try {
  ...
  try {
    ...
    if (resp.ok) {
      const data = await resp.json();
      const protocol = data.find(
        (p) => p.symbol?.toLowerCase() === tokenSymbol.toLowerCase() || p.name?.toLowerCase().includes(tokenSymbol.toLowerCase())
      );
      if (protocol) {
        result.defi_data = {
          name: protocol.name,
    ...
    console.debug("DefiLlama protocol search failed:", e);
  }
  result.summary = {
    sources_used: result.sources_used.length,
    ...
    has_developer_data: result.developer_data?.commit_count_4_weeks !== void 0,
    has_dex_pairs: (result.dex_pairs || []).length > 0,
    has_defi_data: Object.keys(result.defi_data).length > 0
  };
  return result;
}
async function executeChainHealth(chain = "all") {
  const result = {
    chain,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    sources_used: [],
  ...
  const chainsToCheck = chain === "all" ? ["Ethereum", "Solana", "Base", "BSC", "Arbitrum", "Optimism", "Polygon"] : [chain];
  try {
    const resp = await fetch("https://api.llama.fi/chains");
    if (resp.ok) {
      const data = await resp.json();
      for (const chainName of chainsToCheck) {
        const chainData = data.find((c) => c.name.toLowerCase() === chainName.toLowerCase());
        if (chainData) {
  ...
  if (chainsToCheck.includes("Base")) {
    try {
      ...
      console.debug("BaseScan failed:", e);
    }
  }
  try {
    const resp = await fetch("https://mempool.space/api/v1/fees/recommended");
  ...
  result.summary = {
    ...
    total_tvl: Object.values(result.chains).reduce((sum, c) => sum + (c.tvl || 0), 0),
    sources_used: result.sources_used.length,
    health_status: "healthy"
    // Default, could add more logic
  };
  return result;
}
async function executeTwProfile(user) {
  try {
...
async function executeTwSearch(query, limit = 20) {
  try {
    ...
    console.debug("Twitter search failed:", e);
  }
  return { tool: "Twitter Search", query, note: "Twitter API integration pending - search temporarily unavailable", tweets: [] };
}
async function executeSentiment(token, chain) {
  const result = { tool: "Social Sentiment", token, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [] };
  try {
    ...
    console.debug("CoinGecko community failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      if (data.results) {
        ...
        const bullish = recent.filter((r) => r.votes && r.votes.positive > r.votes.negative).length;
        const bearish = recent.filter((r) => r.votes && r.votes.negative > r.votes.positive).length;
  ...
  try {
    ...
    console.debug("DexScreener social failed:", e);
  }
  try {
    const lcKey = "wr70xz6kws7tysb8dxzhdkgemphy8wutq6chfen";
    ...
    console.debug("LunarCrush failed:", e);
  }
...
async function executeHoneypotCheck(tokenAddress, chain) {
  const result = { token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], risk_factors: [], can_buy: null, can_sell: null, honeypot: null };
  try {
    ...
    console.debug("Honeypot.is failed:", e);
  }
  try {
    ...
    console.debug("Token Sniffer failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      if (data.pairs?.length > 0) {
        ...
        result.pair_age_hours = pair.pairCreatedAt ? (Date.now() - pair.pairCreatedAt) / 36e5 : null;
...
async function executePortfolioTracker(wallets, chain) {
  const result = { wallets, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), total_value_usd: 0, portfolios: [], sources_used: [] };
  for (const wallet of wallets) {
    const portfolio = { address: wallet, tokens: [], nfts: [], total_usd: 0 };
    try {
    ...
    try {
      ...
      console.debug("Zerion failed:", e);
    }
    portfolio.total_usd = portfolio.tokens.reduce((sum, t) => sum + (t.value_usd || 0), 0);
    result.portfolios.push(portfolio);
    result.total_value_usd += portfolio.total_usd;
  }
  return result;
}
async function executeCopyTradeFinder(chain, minWinRate, limit) {
  const result = { chain, min_win_rate: minWinRate, limit, timestamp: (/* @__PURE__ */ new Date()).toISOString(), wallets: [], sources_used: [] };
  try {
  ...
  try {
    ...
    console.debug("DeBank rank failed:", e);
  }
  result.wallets = result.wallets.slice(0, limit);
  result.count = result.wallets.length;
  return result;
}
async function executeTokenComparison(tokens, chain) {
  const result = { tokens, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), comparison: [], sources_used: [] };
...
async function executeRiskMonitor(tokenAddress, wallet, chain) {
  const result = { token: tokenAddress, wallet, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), alerts: [], risk_level: "unknown", sources_used: [] };
  if (tokenAddress) {
    try {
      const resp = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${tokenAddress}`);
      if (resp.ok) {
        ...
        if (data.pairs?.length > 0) {
          ...
          if (liq < 1e3) result.alerts.push({ type: "critical", message: "Extremely low liquidity - high rug risk" });
          else if (liq < 1e4) result.alerts.push({ type: "warning", message: "Low liquidity - moderate rug risk" });
          if (pair.locked) {
  ...
  if (wallet) {
    try {
      ...
      console.debug("Wallet check failed:", e);
    }
  }
  result.risk_level = result.alerts.some((a) => a.type === "critical") ? "critical" : result.alerts.some((a) => a.type === "warning") ? "warning" : "safe";
  return result;
}
async function executeDefiYieldScanner(chain, minApy, minTvl) {
  const result = { chain, min_apy: minApy, min_tvl: minTvl, timestamp: (/* @__PURE__ */ new Date()).toISOString(), pools: [], sources_used: [] };
  try {
    ...
    if (resp.ok) {
      ...
      result.pools.sort((a, b) => b.tvl_usd - a.tvl_usd);
      result.pools = result.pools.slice(0, 50);
      result.sources_used.push("defillama");
    }
  } catch (e) {
    console.debug("DefiLlama yields failed:", e);
  }
  result.count = result.pools.length;
  return result;
}
async function executeNftWashDetector(collection, chain) {
  const result = { collection, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), wash_score: 0, flags: [], sources_used: [] };
  if (collection) {
    try {
      ...
      console.debug("OpenSea failed:", e);
    }
  ...
  try {
    ...
    console.debug("NFT market failed:", e);
  }
  result.wash_score = Math.min(result.wash_score, 100);
  result.verdict = result.wash_score > 70 ? "HIGH wash trading risk" : result.wash_score > 40 ? "Moderate wash trading risk" : "Low wash trading risk";
  return result;
}
async function executeBridgeSecurity(bridge, chainFrom, chainTo) {
  const result = { bridge, chain_from: chainFrom, chain_to: chainTo, timestamp: (/* @__PURE__ */ new Date()).toISOString(), bridges: [], sources_used: [] };
  try {
    ...
    console.debug("DefiLlama bridges failed:", e);
  }
  try {
    ...
    console.debug("Rekt failed:", e);
  }
  result.bridge_count = result.bridges.length;
  return result;
}
async function executeGasForecast(chain) {
  const result = { chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), forecast: {}, sources_used: [] };
  if (chain === "all" || chain === "ethereum") {
    try {
      ...
      console.debug("Etherscan gas failed:", e);
    }
  }
  if (chain === "all" || chain === "solana") {
    try {
      ...
      console.debug("Solana gas failed:", e);
    }
  }
  if (chain === "all" || chain === "base") {
    try {
      ...
      console.debug("BaseScan gas failed:", e);
    }
  }
...
async function executeSniperAlert(chain, minLiquidity) {
  const result = { tool: "Sniper Alert", chain, min_liquidity: minLiquidity, timestamp: (/* @__PURE__ */ new Date()).toISOString(), pairs: [], sources_used: [] };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/search?q=");
    if (resp.ok) {
      const data = await resp.json();
      const pairs = (data.pairs || []).filter((p) => {
        const age = p.pairCreatedAt ? (Date.now() - p.pairCreatedAt) / 36e5 : 999;
      ...
      for (const p of pairs) {
        result.pairs.push({
          ...
          liquidity_usd: p.liquidity?.usd || 0,
          price_usd: p.priceUsd,
          age_hours: p.pairCreatedAt ? ((Date.now() - p.pairCreatedAt) / 36e5).toFixed(2) : null,
          dex: p.dexId,
          risk_score: (p.liquidity?.usd || 0) < 5e3 ? 8 : (p.liquidity?.usd || 0) < 2e4 ? 5 : 2
        });
      }
      result.sources_used.push("dexscreener");
    }
  } catch (e) {
    console.debug("DexScreener sniper failed:", e);
  }
  if (chain === "solana" || chain === "all") {
    try {
        ...
        for (const c of (data || []).slice(0, 10)) {
          ...
          if (mc >= minLiquidity) {
            result.pairs.push({
              ...
              chain: "solana",
              liquidity_usd: mc,
              price_usd: c.virtualTokenReserveInSol / c.virtualTokenReserve,
              age_hours: "new",
              dex: "pumpfun",
              risk_score: mc < 1e4 ? 9 : 6
            });
          }
        }
        result.sources_used.push("pumpfun");
      ...
      console.debug("PumpFun sniper failed:", e);
    }
  }
  result.pair_count = result.pairs.length;
  return result;
}
async function executeLiquidityFlow(chain, minAmount) {
  const result = { tool: "Liquidity Flow", chain, min_amount: minAmount, timestamp: (/* @__PURE__ */ new Date()).toISOString(), flows: [], sources_used: [] };
  try {
    const resp = await fetch("https://api.llama.fi/protocols");
    if (resp.ok) {
      const data = await resp.json();
      const bigChanges = data.filter((p) => {
        const tvl = p.tvl || 0;
    ...
    console.debug("DefiLlama flow failed:", e);
  }
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens");
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs) {
        const largePairs = data.pairs.filter((p) => (p.liquidity?.usd || 0) > minAmount).slice(0, 10);
    ...
    console.debug("DexScreener flow failed:", e);
  }
  result.flow_count = result.flows.length;
  return result;
}
async function executeRugPullPredictor(tokenAddress, chain) {
  const result = { tool: "Rug Pull Predictor", token: tokenAddress, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], risk_factors: [], rug_probability: 0 };
  try {
    const resp = await fetch("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress);
    if (resp.ok) {
      const data = await resp.json();
      if (data.pairs?.length > 0) {
        ...
        const ageHours = pair.pairCreatedAt ? (Date.now() - pair.pairCreatedAt) / 36e5 : 999;
        if (liq < 5e3) {
          result.risk_factors.push("Low liquidity: $" + liq);
          result.rug_probability += 25;
        }
        if (ageHours < 24) {
          result.risk_factors.push("Very new pair: " + ageHours.toFixed(1) + "h old");
          result.rug_probability += 20;
        }
        if (volume > liq * 10) {
          result.risk_factors.push("Volume/liquidity ratio extreme: " + (volume / liq).toFixed(1) + "x");
          result.rug_probability += 15;
        }
        if (!pair.locked) {
          result.risk_factors.push("Liquidity NOT locked");
          result.rug_probability += 25;
        }
    ...
    console.debug("DexScreener rug check failed:", e);
  }
  try {
    ...
    if (resp.ok) {
      ...
      if (data.honeypot === true) {
        result.risk_factors.push("HONEYPOT DETECTED");
        result.rug_probability += 50;
      }
      if (data.buyTax > 10 || data.sellTax > 10) {
        result.risk_factors.push("High transfer taxes");
        result.rug_probability += 15;
      }
      result.sources_used.push("honeypot.is");
    }
  } catch (e) {
    console.debug("Honeypot check failed:", e);
  }
  result.rug_probability = Math.min(result.rug_probability, 100);
  result.verdict = result.rug_probability > 70 ? "HIGH RUG RISK - Avoid" : result.rug_probability > 40 ? "MODERATE RUG RISK - Caution" : result.rug_probability > 20 ? "LOW RUG RISK - Monitor" : "SAFE - Low risk indicators";
  return result;
}
async function executeAirdropFinder(wallet, chain) {
  const result = { tool: "Airdrop Finder", wallet, chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), eligible: [], sources_used: [] };
  if (!wallet) return { error: "wallet address required" };
  try {
    const resp = await fetch("https://api.llama.fi/protocols");
    if (resp.ok) {
      const data = await resp.json();
      const potentialAirdrops = data.filter((p) => {
        const hasToken = p.tokenSymbol && p.tokenSymbol !== "-";
        const tvl = p.tvl || 0;
        const category = (p.category || "").toLowerCase();
        return !hasToken && tvl > 1e7 && ["defi", "dex", "lending", "bridge", "yield"].includes(category);
      }).slice(0, 20);
      for (const p of potentialAirdrops) {
        result.eligible.push({
          ...
          category: p.category,
          chains: p.chain,
          likelihood: p.tvl > 1e8 ? "high" : "medium",
    ...
    console.debug("DefiLlama airdrop failed:", e);
  }
  try {
    const resp = await fetch("https://api.zerion.io/v1/wallets/" + wallet + "/positions/?currency=usd");
    if (resp.ok) {
      const data = await resp.json();
      result.wallet_positions = (data.data || []).slice(0, 10).map((p) => ({
        protocol: p.attributes?.protocol_name,
        value_usd: p.attributes?.value,
        chain: p.attributes?.chain
      }));
      result.sources_used.push("zerion");
    }
  } catch (e) {
    console.debug("Zerion wallet check failed:", e);
  }
  result.eligible_count = result.eligible.length;
  return result;
}
async function executeMevProtection(txHash, tokenAddress, chain, slippage) {
  const result = { tool: "MEV Protection", chain, timestamp: (/* @__PURE__ */ new Date()).toISOString(), sources_used: [], risk_assessment: {} };
  if (chain === "base" || chain === "ethereum" || chain === "all") {
    try {
      ...
      console.debug("Etherscan MEV check failed:", e);
    }
  }
  if (tokenAddress) {
    try {
      ...
      if (resp.ok) {
        ...
        if (data.pairs?.length > 0) {
          const pair = data.pairs[0];
          const liq = pair.liquidity?.usd || 0;
          const recommendedSlippage = liq < 1e4 ? 5 : liq < 5e4 ? 3 : liq < 2e5 ? 1.5 : 0.5;
          result.risk_assessment.slippage = {
            current_setting: slippage,
            recommended: recommendedSlippage,
            liquidity_usd: liq,
            risk: slippage > recommendedSlippage ? "HIGH - Reduce slippage" : "OK"
          };
        }
      }
      result.sources_used.push("dexscreener");
    } catch (e) {
      console.debug("DexScreener MEV check failed:", e);
    }
  }
...
async function verifyPayment(x402Payload, env, paymentRequired, toolName) {
  try {
    if (env.NETWORK.includes("solana")) {
      const parsed = JSON.parse(x402Payload);
      const txSignature = parsed.payload?.signature || parsed.signature || (parsed.accepted ? parsed.payload?.signature : null);
      if (!txSignature) {
        console.error("No tx signature found in Solana payload");
        return false;
      }
      ...
      if (!rpcData.result) {
        console.error("Tx not found:", txSignature);
        return false;
      }
      if (rpcData.result.meta?.err) {
        console.error("Tx failed:", rpcData.result.meta.err);
        return false;
      }
      const ourWallet = env.PAY_TO;
      const USDC_MINT = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v";
      const postBalances = rpcData.result.meta?.postTokenBalances || [];
      let received = 0;
      for (const bal of postBalances) {
        if (bal.owner === ourWallet && bal.mint === USDC_MINT) {
          const preBal = (rpcData.result.meta?.preTokenBalances || []).find((b) => b.owner === ourWallet && b.mint === USDC_MINT);
          const preAmt = preBal ? parseInt(preBal.uiTokenAmount.amount) : 0;
          const postAmt = parseInt(bal.uiTokenAmount.amount);
          received = postAmt - preAmt;
        }
      }
      if (received > 0) {
        console.log(`Solana payment verified: ${received} lamports received`);
        return true;
      }
      console.error("No USDC received by our wallet");
      return false;
    }
    const body = { payload: x402Payload, network: "base" };
    ...
    if (resp.ok) {
      ...
      if (data.verified === true) {
        try {
          ...
          console.error("Receipt store failed:", e);
        }
        return true;
      }
    }
    const errText = await resp.text().catch(() => "");
    console.error("Verification failed:", resp.status, errText);
    return false;
  } catch (e) {
...
function fmt(t) {
  return { type: "function", function: { name: "rmi_" + t.name, description: "[" + t.category + "] " + t.description + " - " + t.price + "/call", parameters: { type: "object", properties: { address: { type: "string" }, chain: { type: "string" } } } } };
}
function fmtA(t) {
  return { name: "rmi_" + t.name, description: "[" + t.category + "] " + t.description + " - " + t.price + "/call", input_schema: { type: "object", properties: { address: { type: "string" }, chain: { type: "string" } } } };
}
function buildX402Endpoints(env) {
  const gw = env.GW_URL;
  const ep = {};
  for (const [, t] of Object.entries(RMI_TOOLS)) {
    ep[gw + "/tools/" + t.name] = { price: t.price, priceAtomic: t.priceAtomic, description: t.description, method: t.method, pricing: "from $0.01 USDC \u2014 189 tools across 28 services", category: t.category };
  }
  return ep;
}
function buildPricingHTML(env, allMcp) {
  const gw = env.GW_URL;
  ...
  for (const t of tools) {
    toolCards += '<div class="card"><div class="category cat-' + t.category + '">' + t.category + "</div><h3>" + t.name + "</h3><p>" + (t.description || "") + '</p><div class="price">' + t.price + ' <span>/ call</span></div><div style="margin-top:12px"><a href="' + gw + "/tools/" + t.name + '" style="font-size:13px;color:#00d4ff">Try it -></a></div></div>';
  }
  let mcpItems = "";
  for (const [name, ts] of Object.entries(allMcp)) {
    mcpItems += '<div class="mcp-service"><span>' + name + "</span> " + ts.length + " tools</div>";
  }
...
var UA_BOT = "Discordbot/2.0";
var TW_PROFILE_SOURCES = [
  // 1. fxtwitter (when alive)
  {
    id: "fxtwitter",
    url: (h) => `https://api.fxtwitter.com/${h}`,
  ...
  // 2. vxtwitter (fixed URL)
  {
    id: "vxtwitter",
    url: (h) => `https://api.vxtwitter.com/${h}`,
    headers: { "User-Agent": UA_BOT },
    parse: (b) => {
      try {
        const d = JSON.parse(b);
  ...
  // 3. x.com direct scrape — extract from meta tags
  {
    id: "x_com",
    url: (h) => `https://x.com/${h}`,
  ...
  // 4. xcancel (nitter alternative)
  {
    id: "xcancel",
    url: (h) => `https://xcancel.com/${h}`,
    ...
    parse: (html) => {
      ...
      return {
        ...
        handle: "",
        bio: bio.replace(/View .* on X.*/, "").trim(),
        followers: 0,
        following: 0,
        tweets: 0,
        verified: false
  ...
  {
    id: "nitter_uno",
    url: (h) => `https://nitter.uno/${h}`,
    headers: { "User-Agent": UA_DESKTOP },
    parse: (html) => {
      const title = html.match(/<title>([^<]+)<\/title>/)?.[1] || "";
      if (!title || html.includes("User not found")) return null;
      return { name: title.split("(")[0].trim(), handle: "", bio: "", followers: 0, following: 0, tweets: 0, verified: false };
    }
  }
];
var TW_TIMELINE_SOURCES = [
  // 1. x.com direct scrape — extract tweets from page
  {
    id: "x_com_timeline",
    url: (h) => `https://x.com/${h}`,
    ...
    parse: (html) => {
      ...
      if (stateMatch) {
        try {
          ...
          for (const [, t] of Object.entries(tweets_ents)) {
            if (t.full_text || t.text) {
              tweets.push({
                ...
                url: `https://x.com/status/${t.id_str || ""}`,
                has_media: (t.entities?.media?.length || 0) > 0
              });
            }
          }
        } catch {
  ...
  // 2. xcancel timeline
  {
    ...
    parse: (html) => {
      ...
      let m;
      while ((m = tweetRegex.exec(html)) !== null && tweets.length < 20) {
  ...
  // 3. syndication (when alive)
  {
    id: "syndication",
    url: (h) => `https://syndication.twitter.com/srv/timeline-profile/screen-name/${h}`,
    ...
    parse: (b) => {
      ...
      try {
        ...
        const tweets = entries.filter((x) => x.type === "tweet").map((x) => {
...
var TW_SEARCH_SOURCES = [
  // 1. x.com search scrape
  {
    id: "x_com_search",
    url: (_h, _t, q) => `https://x.com/search?q=${encodeURIComponent(q || "")}&f=top`,
    headers: { "User-Agent": UA_MOBILE, "Accept-Language": "en-US,en;q=0.9" },
    parse: (html) => {
      const tweets = [];
      const stateMatch = html.match(/window\.__INITIAL_STATE__\s*=\s*({.*?});\s*<\/script>/s);
  ...
  // 2. syndication search
  {
    id: "syndication-search",
    url: (_h, _t, q) => `https://syndication.twitter.com/srv/timeline-search?q=${encodeURIComponent(q || "")}`,
    headers: { "User-Agent": UA_DESKTOP },
    parse: (b) => {
      const m = b.match(/<script id="__NEXT_DATA__"[^>]*>(.*?)<\/script>/s);
      if (!m) return null;
...
async function guaranteeFetch(toolType, params) {
  let sources;
  ...
  for (const source of sources) {
    try {
      ...
      if (resp.status >= 400) {
        results.push({ method: source.id, status: "http_error", http_status: resp.status });
        continue;
      }
...
async function handleRequest(request, env) {
  ...
  if (path === "/diagnose-mcp") {
    let result = { backend_mcp: env.BACKEND_MCP, admin_key_defined: !!env.ADMIN_KEY };
    try {
      const headers = {};
      if (env.ADMIN_KEY) headers["x-admin-key"] = env.ADMIN_KEY;
      const resp = await fetch(env.BACKEND_MCP + "/tools", { headers });
  ...
  if (path === "/glama.json") {
    ...
    return Response.json({
      ...
      version: "2.0.0",
      description: "44 crypto security & intelligence tools + 154 MCP services via x402 micropayments from $0.01/call.",
  ...
  if (path === "/mcp") {
    if (request.method === "POST") {
      try {
        ...
        if (body.method === "tools/list") {
          ...
          const mcpTools = Object.entries(allMcp).flatMap(
            ([svc2, ts]) => ts.map((t) => ({
              name: `${svc2}_${t.name}`,
              title: t.description || `${svc2}_${t.name}`,
              description: t.description || "",
              inputSchema: normalizeSchema(t.parameters),
              outputSchema,
              annotations: { title: t.description || `${svc2}_${t.name}` }
            }))
          );
        ...
        if (body.method === "tools/call") {
          try {
            ...
            for (const [k, v] of Object.entries(args)) {
              ...
              else if (v !== null && v !== void 0) fakeParams.set(k, JSON.stringify(v));
            }
            ...
            if (dotIdx > 0) {
              const svc2 = toolName.substring(0, dotIdx);
              const tn = toolName.substring(dotIdx + 1);
              const tools = allMcp[svc2];
              if (tools) {
                ...
                if (tool) {
                  try {
                    const backendResp = await fetch(env.BACKEND_API + "/api/v1/x402/mcp-proxy", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ service: svc2, tool: tn, arguments: args })
                    });
                    if (backendResp.ok) {
                  ...
                  return respond({ result: { service: svc2, tool: tn, arguments: args, note: "Backend proxy unavailable \u2014 tool metadata only" } });
                }
              }
            }
    ...
    const svc = Object.entries(allMcp).map(([n, ts]) => ({ name: n, toolCount: ts.length }));
    ...
    if (parts.length >= 2) {
      ...
      if (tools) {
        if (tn) {
          ...
          if (t) return Response.json({ service: svc, tool: t });
        }
        return Response.json({ service: svc, tools });
      }
    }
  }
  if (path === "/llms.txt") {
    ...
    const lines = [
      ...
      `- Smithery: https://smithery.ai/server/@cryptorugmunch/x402-base`,
      "",
      "## Pricing",
      "44 crypto security & intelligence tools + 154 MCP services. Instant on-chain micropayments from $0.01 USDC. No signup, no API keys, no limits.",
      "",
      "## Tools"
    ];
  ...
  if (path === "/anthropic-tools") {
    const tools = [...Object.values(RMI_TOOLS).map(fmtA), ...Object.entries(allMcp).flatMap(([svc, ts]) => ts.map((t) => ({ name: svc + "_" + t.name, description: t.description, input_schema: t.parameters || { type: "object", properties: {} } })))];
    return Response.json({ provider: "anthropic", rmi_tools: Object.keys(RMI_TOOLS).length, total: tools.length, tools });
  }
  if (path === "/gemini-tools") {
    const decls = [...Object.values(RMI_TOOLS).map(fmtA), ...Object.entries(allMcp).flatMap(([svc, ts]) => ts.map((t) => ({ name: svc + "_" + t.name, description: t.description, parameters: t.parameters || { type: "object", properties: {} } })))];
    return Response.json({ provider: "gemini", rmi_tools: Object.keys(RMI_TOOLS).length, total: decls.length, tools: { function_declarations: decls } });
  }
  if (path === "/langchain-tools") {
    const tools = [...Object.values(RMI_TOOLS).map(fmtA), ...Object.entries(allMcp).flatMap(([svc, ts]) => ts.map((t) => ({ name: svc + "_" + t.name, description: t.description, parameters: t.parameters || { type: "object", properties: {} } })))];
    return Response.json({ provider: "langchain", rmi_tools: Object.keys(RMI_TOOLS).length, total: tools.length, tools });
  }
  if (path === "/mcp-tools") {
    const mcpSection = Object.entries(allMcp).map(([name, ts]) => ({ name, toolCount: ts.length }));
    return Response.json({ provider: "mcp", rmi_tools: Object.values(RMI_TOOLS).map((t) => ({ name: t.name, price: t.price, category: t.category })), mcp_tools: mcpSection, totalTools: Object.keys(RMI_TOOLS).length + Object.values(allMcp).flat().length });
  }
  ...
  if (path.startsWith("/tools/")) {
    ...
    if (x402Payload) {
      ...
      const paymentRequired2 = request.headers.get("PAYMENT-REQUIRED");
      const verified = await verifyPayment(x402Payload, env, paymentRequired2 || void 0, toolName);
  ...
  if (path.startsWith("/proxy/")) {
    ...
    try {
      return await fetch(backendUrl, { headers: request.headers, method: request.method, body: request.body });
    } catch (e) {
